...
def process():
    ...
    print('cpu:',aidlite.NNModel(model_path,inShape1,outShape1,4,0))
        ...
        if bFace:
            for i in range(len(detections)):
                ...
                if bfirstframe:
                    ...
                    roi_orifirst=faceswap(spoints,fpoints,roi_ori,roi_orifirst)
                ...
                shape=frame.shape
                global saveimg
                # print('x0,y0,x1,y1:',int(y-shape[0]/2),int(y+shape[0]/2), int(x-shape[1]/2),int(x+shape[1]/2))
                saveimg=img_pad[max(0,int(y-shape[0]/2)):int(y+shape[0]/2), max(0,int(x-shape[1]/2)):int(x+shape[1]/2)]
