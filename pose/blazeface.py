...
def blazeface(raw_output_a, raw_output_b, anchors):
    ...
    assert(raw_box_tensor.size==896*12)
    raw_score_tensor = raw_score_tensor.reshape(1,896,1)
    raw_box_tensor = raw_box_tensor.reshape(1,896,12)
...
def convert_to_orig_points(results, orig_dim, letter_dim):
    ...
    results[:, 4:10:2] = (results[:, 4:10:2] - offset_x) * scale_x
    results[:, 5:11:2] = (results[:, 5:11:2] - offset_y) * scale_y
    #converting from 0-1 range to (orign_dim) range
    results[:, 0:12:2] *= orig_dim[1]
    results[:, 1:13:2] *= orig_dim[0]
