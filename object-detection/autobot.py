...
def real_time_object_detection(droid,input_details,interpreter, colors,output_details,class_names):
    ...
    while True:
        ...
        if frame is not None:
            ...
            # Draw bounding boxes on the image file
            result = draw_boxes_and_Control(droid,frame, out_scores, out_boxes, out_classes, class_names, colors)
            end = time.time()
...
def main():
    ...
    real_time_object_detection(droid,input_details,interpreter, colors,output_details,class_names)
...
class MyApp(App):
    ...
    def main(self):
        ...
        main_container = VBox(width=360, height=360, style={'margin':'0px auto'})
        main_container.css_width = "98%"
        # main_container.css_height = "80%"
        
        self.aidcam = OpencvVideoWidget(self, width=360, height=360)
