...
def draw_boxes_and_Control(droid,image, out_scores, out_boxes, out_classes, class_names, colors):
    h, w, _ = image.shape
    ...
    for i, c in reversed(list(enumerate(out_classes))):
        ...
        leftControl=0.0
        rightControl=0.0
        if predicted_class=='person' and score > 0.5 :
        ...
        droid.sendControlToVehicle(int(leftControl*92),int(rightControl*92))
...
def draw_boxes_and_turn(image, out_scores, out_boxes, out_classes, class_names, colors):
    h, w, _ = image.shape
    leftControl=0.0
    rightControl=0.0
    print("w,h:",w,h)
